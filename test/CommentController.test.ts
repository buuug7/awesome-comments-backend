...
describe('Test CommentController', () => {
  ...
  test('GET /comments/1/reply', async () => {
    const response = await request(app.callback())
      .post(`${apiPrefix}/comments/1/reply`)
      .set('Authorization', `Bearer ${token}`)
      .send({
  ...
  test('GET /comments/1/star', async () => {
    const response = await request(app.callback())
      .post(`${apiPrefix}/comments/${createdComment.id}/star`)
      .set('Authorization', `Bearer ${token}`);
  ...
  test('GET /comments/1/unStar', async () => {
    const response = await request(app.callback())
      .post(`${apiPrefix}/comments/${createdComment.id}/unStar`)
      .set('Authorization', `Bearer ${token}`);
  ...
  test('GET /comments/1/starCount', async () => {
    const response = await request(app.callback())
      .get(`${apiPrefix}/comments/1/starCount`)
      .set('Authorization', `Bearer ${token}`);
  ...
  test('GET /comments/1/starUsers', async () => {
    const response = await request(app.callback())
      .get(`${apiPrefix}/comments/1/starUsers`)
      .set('Authorization', `Bearer ${token}`);
